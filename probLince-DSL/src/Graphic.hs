...
plotDiscontinuityPoints (r,g,b) points = toPlot
                                       $ plot_points_title .~ ""
                                       $ plot_points_values .~ Prelude.map ((/1000000.0) . fromIntegral >< id) points
...
trajectoriesToGraphic :: String -> Either String [Map String [[(Int, Double)]]] -> IO ()
trajectoriesToGraphic configFile trjs = do
                content <- A.decode <$> (BS.readFile configFile)
                case content of 
                    Nothing -> putStrLn "Invalid JSON config file"
                    Just conf -> either print (graphic conf) trjs
        where
            graphic conf trajectories = makeGraph conf 
